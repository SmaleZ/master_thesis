...
for skill in [5,6,7,8,9]:
    continuetrain_env = DIAYN_Pretrained_Wrapper(env, skill_choosen=skill)
    continue_train_agent = SAC.load("trainedmodel/pretrained_HalfCheetah-v2-1000000.zip", env=continuetrain_env)
    continue_train_agent.learn(total_timesteps=total_supervised_steps)
    continutrained_path = "trainedmodel/continualtrained_{}-{}-skill{}".format("HalfCheetah-v2", total_unsupervised_steps, skill)
